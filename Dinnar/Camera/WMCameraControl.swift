...
class WMCameraControl: UIView {
    ...
    override init(frame: CGRect) {
        ...
        retakeButton.setBackgroundImage(UIImage.imageWithName(named: "icon_return"), for: .normal)
        ...
        takeButton.setBackgroundImage(UIImage.imageWithName(named: "icon_finish"), for: .normal)
        takeButton.addTarget(self, action: #selector(takeButtonClick), for: .touchUpInside)
        self.addSubview(takeButton)


        exitButton.setImage(UIImage.imageWithName(named: "arrow_down"), for: .normal)
...
//        self.addSubview(changeCameraButton)
    ...
    @objc func retakeButtonClick() {
        ...
        takeButton.frame = cameraButton.frame
        recordTime = 0
        takeButton.isUserInteractionEnabled = true
        guard let delegate = delegate else { return }
    ...
    @objc func takeButtonClick() {
        guard let delegate = delegate else { return }
        takeButton.isUserInteractionEnabled = false
